...
class DynastyPrivacyWebVC: UIViewController, WKScriptMessageHandler, WKNavigationDelegate, WKUIDelegate {
    @IBOutlet weak var privacyBackButton: UIButton!
    ...
    override func viewDidLoad() {
        ...
        self.privacyData = UserDefaults.standard.array(forKey: UIViewController.dynasty_GetUserDefaultKey())
        dynasty_InitSubViews()
        dynasty_InitNavView()
        dynasty_InitWebView()
        dynasty_StartLoadWebView()
    }
    ...
    */
    ...
    // MARK: - INIT
    private func dynasty_InitSubViews() {
    ...
    private func dynasty_InitWebView() {
    ...
    private func dynasty_StartLoadWebView() {
    ...
    private func dynasty_ReloadWebViewData(_ adurl: String) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.1) {
            if let storyboard = self.storyboard,
               let adView = storyboard.instantiateViewController(withIdentifier: "DynastyPrivacyWebVC") as? DynastyPrivacyWebVC {
                adView.url = adurl
           ...
           let trackMessage = message.body as? [String: Any] {
            ...
            if let ty = confData[18] as? Int, ty == 1 {
                    ...
                    do {
                        if let jsonObject = try JSONSerialization.jsonObject(with: data) as? [String: Any] {
                            if tName != (confData[8] as? String) {
                                dynasty_SendEvent(tName, values: jsonObject)
                                return
                            }
                            ...
                            if let adId = jsonObject["url"] as? String, !adId.isEmpty {
                                dynasty_ReloadWebViewData(adId)
                            }
                        }
                    } catch {
                        dynasty_SendEvent(tName, values: [tName: data])
                    }
                } else {
                    dynasty_SendEvent(tName, values: [tName: tData])
                }
            } else if let ty = confData[18] as? Int, ty == 2 {
                dynasty_SendEvents(tName, paramsStr: tData)
            } else {
                if tName == confData[28] as? String {
                    ...
                    dynasty_SendEvent(withName: tName, value: tData)
                }
            }
            
        } else if name == (confData[19] as? String) {
            if let messageBody = message.body as? String,
               let dic = dynasty_JsonToDic(withJsonString: messageBody) as? [String: Any],
               let evName = dic["funcName"] as? String,
               let evParams = dic["params"] as? String {
                
                if evName == (confData[20] as? String) {
                    if let uDic = dynasty_JsonToDic(withJsonString: evParams) as? [String: Any],
                    ...
                    dynasty_endEvents(withParams: evParams)
                }
            }
        }
    }
